...
next_Param_table = next_Param_table(1:5,:);
% 
...
for j = 1:Value_Variation
    ...
    Search_Traffic_name_cell = cellstr(strtrim(char(regexp(TestRun,'[^\n]*Traffic.0.Name =[^\n]*','match'))));
    Search_Traffic_name_cell_split = cellfun(@(v) strsplit(v,' = '),Search_Traffic_name_cell,'UniformOutput',false);
    Traffic_name_cell = cellfun(@(v) v{2},Search_Traffic_name_cell_split,'UniformOutput',false);

    if sum(contains(Traffic_name_cell,'RV')) > 0

        Traffic_name = 'RV';

        eval(['T00_tx(:,1) = data.Traffic_' Traffic_name '_tx.data;'])
        eval(['T00_ty(:,1) = data.Traffic_' Traffic_name '_ty.data;'])
        eval(['T00_tz(:,1) = data.Traffic_' Traffic_name '_tz.data;'])

        eval(['T00_rx(:,1) = data.Traffic_' Traffic_name '_rx.data;'])
        eval(['T00_ry(:,1) = data.Traffic_' Traffic_name '_ry.data;'])
        eval(['T00_rz(:,1) = data.Traffic_' Traffic_name '_rz.data;'])

        eval(['tmp_Traffic_csv_table=table(T00_tx,' 'T00_ty,' 'T00_tz,'...
            'T00_rx,' 'T00_ry,' 'T00_rz);' ])

        Traffic_csv_table = [Traffic_csv_table, tmp_Traffic_csv_table];

        clearvars T00_tx T00_ty T00_tz T00_rx T00_ry T00_rz
...
if isempty(Variable_array)==0
    ...
    [tmp_impact_section,tmp_pre_crash_true,tmp_impact_sample_all,tmp_safe_crash_with_AEB,tmp_Safe_ON,tmp_Safe_OFF,tmp_Crash_ON,tmp_Crash_OFF,tmp_Driving_Time,tmp_Driving_Distance,tmp_info,tmp_Impactspeedresult]...
        =genCollisionGTCar(Data_Path,CarMaker_Project_Path,Cur_AES_Scenario_Selection,next_Param_table);
