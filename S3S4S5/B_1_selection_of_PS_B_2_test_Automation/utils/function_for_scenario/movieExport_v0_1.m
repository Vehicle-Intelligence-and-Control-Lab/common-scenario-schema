function movieExport_v0_1(cur_scenario_selection,mat_path,erg_path,simoutput_path,impact_section)
%% Log
% [ Date: 211224 ]
% Line #: Line35
...
info_impact_section = [[1:length(impact_section)]' impact_section];
list_safe_scenario = info_impact_section(impact_section(:,2) == 0,1);
list_crash_scenario = info_impact_section(impact_section(:,2) ~= 0,1);

% first_safe_scenario=find(impact_section(:,1)==0,1);
% first_safe_scenario(isempty(first_safe_scenario))=0;
% first_crash_scenario=find(impact_section(:,1)~=0,1);
% first_crash_scenario(isempty(first_crash_scenario))=0;

if ~isempty(list_safe_scenario)
    first_safe_scenario = list_safe_scenario(1);
else
    first_safe_scenario = 0;
end

if ~isempty(list_crash_scenario)
    first_crash_scenario = list_crash_scenario(1);
else
    first_crash_scenario = 0;
end
